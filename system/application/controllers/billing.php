...
where firm_id={$this->uri->segment(3)} and data_finish is null";
    ...
    public function pre_counter_to_check()
    {
        $this->left();
        $this->load->view("other_reports/counter_to_check/pre");
        $this->load->view("right");
    }
    ...
    public function counter_to_check()
    {
        if ($_POST['year_diff'] >= 0) {
            $data['year_diff'] = $_POST['year_diff'];
            $this->db->where("year_diff >", $_POST['year_diff']);
            $data['report'] = $this->db->get("industry.counter_to_check")->result();
            if (isset($_POST['to_excel'])) {
                $this->export_to_excel("other_reports/counter_to_check/report", $data, 'Наряд-задание по ТУРЭ');
            } else {
                $this->load->view("other_reports/counter_to_check/report", $data);
            }
        }
    }	
	...
	public function export_rekvizit_schet()
    {
        ...
        if ($db) {
            ...
            if (!copy("c:/oplata/rschet.dbf", "c:/oplata/arykb{$month}.dbf")) {
